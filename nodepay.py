...
def main():
    ...
    # Try to read tokens from tokens.txt
    try:
        with open('tokens.txt', 'r') as file:
            tokens = [line.strip() for line in file if line.strip()]
    except FileNotFoundError:
        print("File tokens.txt not found. Please enter your token manually.")

    # If no tokens are found in the file, ask the user to enter a token manually
    if not tokens:
